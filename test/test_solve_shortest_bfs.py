...
from funmaze.solve.shortest_bfs import solve_shortest_bfs


def test_shortest_1() -> None:
    maze: Graph[int] = {(0, 1), (1, 2), (0, 2)}
    sols = list(tuple(sol) for sol in solve_shortest_bfs(maze, 0, 2))
    assert len(sols) == 1
    assert sols[0] == (0, 2)
...
def test_shortest_3(shape: tuple[int, ...]) -> None:
    ...
    solutions = list(solve_shortest_bfs(maze, start, end))
...
def test_shortest_4() -> None:
    ...
    sols = list(frozenset(sol) for sol in solve_shortest_bfs(maze, 0, 5))
    assert not sols
