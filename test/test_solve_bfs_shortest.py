...
def test_shortest_1() -> None:
    ...
    sols = list(tuple(sol) for sol in solve_bfs_one_shortest(maze, 0, 2))
    assert len(sols) == 1
    assert sols[0] == (0, 2)
...
def test_shortest_3(shape: tuple[int, ...]) -> None:
    ...
    solutions = list(solve_bfs_one_shortest(maze, start, end))
...
def test_shortest_4() -> None:
    ...
    sols = list(frozenset(sol) for sol in solve_bfs_one_shortest(maze, 0, 5))
    assert not sols
